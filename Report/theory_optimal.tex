...
%  Shape regularity gives the equivalence $h'_K \eqsim h_K$, so that all the bounds in Theorem~\ref{thm:assumptions} are still
...
%  This element is refined, and from properties of the orthogonal projector $\Pi_{p-1}$ we infer that
  ...
  The use of patches instead of elements makes things a little confusing. %We will first analyze the above inequality for a triangle.
  ...
  \end{align*}
%For every patch $\w_a \in \RR^3_{\T \to \T_\star}$, we may use \eqref{eq:singlepatch} to discover a strict decrease oscillation on the refined elements of $\w_a$.
